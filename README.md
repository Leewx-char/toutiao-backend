...
![架构图](assets/architecture.excalidraw)

> 点击上面链接或打开 `assets/architecture.excalidraw` 查看完整架构图（需使用 [Excalidraw](https://excalidraw.com) 打开）。
...
FastAPI 主入口 (main.py)
  ├── CORS 中间件
     ...
     ▼
路由层 (Routers)
  ├── /api/news      — 新闻分类、列表、详情、相关推荐
  ├── /api/user      — 注册、登录、信息获取/修改、密码修改
  ├── /api/favorite  — 收藏检查、添加、删除、列表、清空
  └── /api/history   — 记录添加、列表、删除、清空
     │
     ▼
